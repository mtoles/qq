...
class Primary_Model:
    ...
    def evaluate(self, target_col_name):
        with torch.no_grad():
            str_pred = []
            str_gt = []
            cls_pred = []
            ...
            tokenized_input = self.tk(self.prepped_val_dataset[target_col_name])[
                "input_ids"
            ]
            for i, x in enumerate(tqdm(self.prepped_val_dataset)):
                generation = self.forward(
                    input_ids=torch.Tensor(tokenized_input[i]).cuda().int().unsqueeze(0)
                )
...
class BigBird_PM(Primary_Model):
    ...
    def prepare_data(self, masking_scheme):
        ...
        # In order to minimize confusion, remove input_ids.
        # Explicitly call the tokenizer on the target column wheneve you need some input_ids.
        self.prepped_val_dataset = self.prepped_val_dataset.remove_columns(
            ["input_ids"]
        )
