...
def main(split, dataset, cache_dir):
    """Running area"""
    ...
    if dataset == "natural_questions":
        ...
        # data = datamap(
        data = data[1886:].map(
            prepare_inputs_nq, fn_kwargs=fn_kwargs, cache_file_name=cache_file_name
        )
        ...
        data = data.map(
            prepare_inputs_hp, fn_kwargs=fn_kwargs, cache_file_name=cache_file_name
        )
        print(data)
